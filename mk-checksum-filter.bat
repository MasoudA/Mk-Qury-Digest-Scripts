...
# This program sorts and filters table checksums output by mk-table-checksum,
# and shows only those that differ.
#
...
$o->set('master', '') unless $o->get('master');

my $unique = $o->get('unique');
if ( $unique && $unique !~ m/^(?:host|db|table)$/ ) {
   $o->save_error('--unique must be one of host|db|table');
}
$o->usage_or_errors();
...
my $exit_status = 0;
my $one_file    = @ARGV < 2 && !($o->get('ignore-databases') || %{$o->get('equal-databases')});
...
my %lines_for;          # All lines from all files, !$one_file 
my $last_chunk;         # db.tbl.chunk of last line read
my $current_set = [];   # working set of lines to process

# For use with --unique.
my %unique_items;
my $unique_idx = !$unique          ? undef
               : $unique eq 'host' ? $HOST
               : $unique eq 'db'   ? $DATABASE
               :                     $TABLE;
...
while ( my $line = <> ) {    # Magically reads STDIN or files in @ARGV
   ...
   if ( $one_file && $line =~ m/^DATABASE/ && $o->get('header') ) {
      print $line, "\n";
      next LINE;
   }
   ...
   push @cols, $line;
   my $chunk
      = $o->get('ignore-databases') || $o->get('equal-databases')->{$cols[$DATABASE]}
...
sub process_set {
   ...
   @$set = sort {
           $a->[$HOST] eq $o->get('master') ? -1
         : $b->[$HOST] eq $o->get('master') ? 1
         : $a->[$HOST] cmp $b->[$HOST]
   } @$set;
   ...
   $exit_status = 1 if $is_diff;

   if ( $o->get('verbose') || $is_diff ) {
      foreach my $line (@$set) {
         if ( $unique ) {
...
# Deferred processing: print out unique items
if ( $unique ) {
   foreach my $thing ( sort keys %unique_items ) {
      print $thing, "\n";
   }
}
...
L<http://www.maatkit.org/bugs/mk-checksum-filter>.
...
L<"--ignore-databases"> and L<"--equal-databases"> are mutually exclusive.
...
words, this makes C<database1.table1.chunk1> equal to C<database2.table1.chunk1>
...
For a list of known bugs see L<http://www.maatkit.org/bugs/mk-checksum-filter>.
