...
sub main {
   ...
   if ( my $users = $o->get('only') ) {
      my @users = map {
         ...
         MKDEBUG && _d('Parsed only', $_, 'as user', $user, 'and host', $host);
         { User => $user, Host => $host };
      }
      grep {
         if ( $_ !~ /\@/ ) {
            ...
            MKDEBUG && _d('Will get all grants for', $_, 'on all hosts');
            push @all_hosts, $_;
            0;
         }
         else {
   ...
   if ( my $users = $o->get('ignore') ) {
      my %users = map {
         my ( $user, $host ) = parse_user($_);
         MKDEBUG && _d('Parsed ignore', $_, 'as user', $user, 'and host',$host);
   ...
   foreach my $u ( @$users ) {
      ...
      if ( $ignore_users && $ignore_users->{$user_host} ) {
         MKDEBUG && _d('Ignoring user', $user_host);
         next USER;
      }
      else {
         MKDEBUG && _d('Checking user', $user_host);
      }
      ...
      if ( $o->get('revoke') ) {
         my @revoke = map {
            ...
            my ( $grants, $on_what, $user ) = $grant
               =~ m/GRANT (.*?) ON ((?:`|\*).*?) TO ('[^']*'\@'[^']+')?/;
            MKDEBUG && _d('grants:', $grants, 'on_what:', $on_what,
               'user:', $user);

            if ( !$user ) {
               # Anonymous user: ''@'' (issue 445).
               MKDEBUG && _d('Anonymous user');
               $user = "''\@''";
            }
...
L<http://www.maatkit.org/bugs/mk-show-grants>.
...
For a list of known bugs see L<http://www.maatkit.org/bugs/mk-show-grants>.
