...
# This is mk-slave-move, a program to move slaves around in a replication
# hierarchy (promote, demote, etc).
#
...
sub main {
   ...
   my $slave_dsn    = $dp->parse(shift @ARGV, $dsn_defaults);
   my @dsns;
   push @dsns, $slave_dsn;
   while ( my $arg = shift(@ARGV)  ) {
      my $dsn = $dp->parse($arg, $slave_dsn, $dsn_defaults);
      push @dsns, $dsn;
   }

   if ( !@dsns ) {
      $o->save_error('At least one DSN is required.');
   }
   elsif ( @dsns < 2 && !($o->get('sibling-of-master') || $o->get('detach')) ) {
      $o->save_error('At least two DSNs are required.');
   }
   ...
   if ( $o->get('ask-pass') ) {
      ...
      # Same password for all slave hosts.
      map { $_->{p} ||= $pass } @dsns;
   }

   $slave_dsn = shift @dsns;  # re-using $slave_dsn from above
   my $slave_dbh
      = $dp->get_dbh($dp->get_cxn_params($slave_dsn), { AutoCommit => 1 });
   $dp->fill_in_dsn($slave_dbh, $slave_dsn);

   if ( $o->get('sibling-of-master') ) {
      $ms->make_sibling_of_master($slave_dbh, $slave_dsn, $dp, $o->get('timeout'));
   }
   elsif ( $o->get('slave-of-sibling') ) {
      my $sib_dsn = shift @dsns;
      my $sib_dbh
         = $dp->get_dbh($dp->get_cxn_params($sib_dsn), { AutoCommit => 1 });
      $dp->fill_in_dsn($sib_dbh, $sib_dsn);
      $ms->make_slave_of_sibling(
         $slave_dbh, $slave_dsn, $sib_dbh, $sib_dsn, $dp, $o->get('timeout'));
   }
   elsif ( $o->get('slave-of-uncle') ) {
      my $unc_dsn = shift @dsns;
      my $unc_dbh
         = $dp->get_dbh($dp->get_cxn_params($unc_dsn), { AutoCommit => 1 });
      $dp->fill_in_dsn($unc_dbh, $unc_dsn);
      $ms->make_slave_of_uncle(
         $slave_dbh, $slave_dsn, $unc_dbh, $unc_dsn, $dp, $o->get('timeout'));
   }
   elsif ( $o->get('detach') ) {
      my $stat = $ms->detach_slave($slave_dbh);
      print $ms->short_host($slave_dsn),
         " was a slave of ", $ms->short_host($stat),
         " at position ", $ms->pos_to_string($ms->repl_posn($stat)), "\n";
   }
   else {
      die "Sorry, this functionality isn't implemented yet";
   }
...
L<http://www.maatkit.org/bugs/mk-slave-move>.
...
The hosts are given by a DSN.  A DSN is a special syntax that can be either
just a hostname (like C<server.domain.com> or C<1.2.3.4>), or a
...
defaults from the slave host, so you don't need to specify them in both places.
C<mk-slave-move> reads all normal MySQL option files, such as ~/.my.cnf, so
...
Specify one and only one of L<"--sibling-of-master">, L<"--slave-of-sibling">,
L<"--slave-of-uncle">, or L<"--detach">.
...
For a list of known bugs see L<http://www.maatkit.org/bugs/mk-slave-move>.
